...
public class AboutMeAfterSaleSupportActivity extends SysActivity   {	
	private SysParam TPMSParam ;
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		TPMSParam = ((SysParam)SysParam.getInstance(this));
		TPMSParam.addActivity(this);
		DisplayMetrics metric = new DisplayMetrics();
		getWindowManager().getDefaultDisplay().getMetrics(metric);
		float density = metric.density;
		if(density==1.5){
			setContentView(R.layout.aboutme_rightinfoden15);
		}else {
...
//		Drawable dr = this.getResources().getDrawable(R.drawable.newmain_bg_center);
//		linearLayout.setBackgroundDrawable(dr);
		String info = this.getResources().getString(R.string.afterSaleSupport);
		info += "\n" + getResources().getString(R.string.AppVersion) + "："+getVersion()+getResources().getString(R.string.testtime);
		...
		new SysParam.Builder(this).setTitle(getResources().getString(R.string.SupportInformation)).setBackButton(getResources().getString(R.string.StringAboutMe), new View.OnClickListener(){
			public void onClick(View v){
				
				KeyEvent newEvent = new KeyEvent(KeyEvent.ACTION_DOWN,KeyEvent.KEYCODE_BACK);
				AboutMeAfterSaleSupportActivity.this.onKeyDown(KeyEvent.KEYCODE_BACK,newEvent);
			}
		}).create();
