...
public class WheelView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //绘制选中行
        paint.setColor(getResources().getColor(R.color.gray_fg));
        canvas.drawRect(getPaddingLeft(), centerItemTop, width-getPaddingRight(), centerItemBottom, paint);
      //绘制中间背景
        paint.setColor(getResources().getColor(R.color.theme_color));
        if(isShowPoint){
        	canvas.drawText(".",width-getPaddingRight() - paint.measureText("."),getBaseLine(paint,centerItemTop,itemHeight),paint);
        }
        if(unit != null && !"".equals(unit.trim())){
        	canvas.drawText(unit,width-getPaddingRight() - paint.measureText(unit) + 5,getBaseLine(paint,centerItemTop,itemHeight),paint);
        }
...
//            mScroller.springBack(0,(int)scrollY,0,0,minScrollY,maxScrollY);
    ...
    public void isShowPoint(boolean isShow){
